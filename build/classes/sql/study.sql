...
create table board (
   ...
   readcnt int(10),     -- 조회수
   grp int,             -- 답글 작성시 원글의 게시글 번호(=num)
   grplevel int(3),     -- 답글의 레벨. (답글의 답글의 ...) 0(원글), 1(원글의 답변), 2(원글의 답변의 답변),..//ex. 2번 글의 5번 답글의 7번답글이면 7번답글의 grplevel=2, 5번 답글의 grplevel=1
   grpstep int(5)       -- 답변 그룹의 출력 순서 
);
